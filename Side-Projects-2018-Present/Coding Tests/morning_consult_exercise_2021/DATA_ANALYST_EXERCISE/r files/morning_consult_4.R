...
# c.	Total Favorability: % very favorable + % somewhat favorable
total_fav$`Don't Approve` <- total_fav$`Somewhat Unfavorable`/nrow(x) + total_fav$`Very Unfavorable`/nrow(x)
total_fav$`Other` <- total_fav$`Heard Of, No Opinion`/nrow(x) + total_fav$`Never Heard Of`/nrow(x)
total_fav <- total_fav[-c(2:9)]
colnames(total_fav)[2] <- "Approve"

total_fav_long <- gather(total_fav, "Position", "Rating", 2:ncol(total_fav))

barplot(Rating ~ Figures, data = subset(total_fav_long, total_fav_long$Position == "Approve"))
